...
useEffect(() => {
  ...
  function submitEdits(newtodo) {
    const updatedTodos = [...todos].map((todo) => {
      ...
      setTodos(updatedTodos);
      setTodoEditing(null); // Reset the to-do editing state
    }
...
{todos.map((todo) => (
  ...
  <div key={todo.id} className="todo">
    ...
    <div className="todo-actions">
      ...
      ) : /*':' the breakpoint between if (?) and else */
      (
        <button onClick={() => setTodoEditing(todo.id)}>Edit</button> //update the to-do update state
      ) }  
